...
{
    public class Group_Instruction : IInstruction
    {
        ...
        public Group_Instruction(ITelegramBotClient telegramBotClient, CallbackQueryEventArgs callbackQueryEventArgs, BotPlatform botPlatform)
        {
        ...
        public void Execute()
        {
            ...
            for (int i = 0; i < rss.ResponseData.Count; i++)
                keyboardButtonsListrss[i] = new InlineKeyboardButton[] { InlineKeyboardButton.WithCallbackData(rss.ResponseData[i].AliasName, string.Concat("Add_Group_Rss_", rss.ResponseData[i].Id)) };
            ...
            "last step to attach rss to group. Choose one of the below.",
            replyMarkup: new InlineKeyboardMarkup(keyboardButtonsListrss)).GetAwaiter();

            SessionHelper.GetSession(_callbackQueryEventArgs.CallbackQuery.From.Id).ChannelId = Convert.ToInt32(_callbackQueryEventArgs.CallbackQuery.Data.Substring(6, _callbackQueryEventArgs.CallbackQuery.Data.Length - 6));
        }
    }
}
