    ...
    public class Group_Instruction : IInstruction
    {
        ...
        public void Execute()
        {
            ...
            for (int i = 0; i < rss.ResponseData.Count; i++)
                keyboardButtonsListrss[i] = new InlineKeyboardButton[] { InlineKeyboardButton.WithCallbackData(rss.ResponseData[i].Url, string.Concat("Add_Group_Rss_", rss.ResponseData[i].Url)) };
            ...
            SessionHelper.GetSession(_callbackQueryEventArgs.CallbackQuery.From.Id).Channel = _callbackQueryEventArgs.CallbackQuery.Data.Substring(6, _callbackQueryEventArgs.CallbackQuery.Data.Length - 6);
        }
    }
}
